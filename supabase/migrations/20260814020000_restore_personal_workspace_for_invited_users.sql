-- Invited users should keep the same personal workspace bootstrap as direct
-- signups. The invitation acceptance flow adds the invited workspace after
-- the auth link is verified, so the user ends up with both workspaces.
